...
- [ ] Create a `lib/helpers/` directory for shared utility functions
- [ ] Extract `_normalize_team_id()` into a shared helper (currently duplicated in `fetch_team_box_scores.py` and `fetch_team_game_logs.py`)
- [ ] Create a shared CSV writing utility that handles directory creation, error handling, and consistent logging
